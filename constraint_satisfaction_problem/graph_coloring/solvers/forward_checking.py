...
class GraphColoringBacktracking(GraphColoringSolver):
    ...
    def solve_forward_checking(self):
        ...
        while np.amin(self.nodes_colors) == -2:
            ...
            min_colors_size += 1
            self.nodes_colors_two_dim = np.zeros(shape=(self.nodes_size, min_colors_size))
            self.solve_forward_checking_rec(0, min_colors_size, np.copy(nodes_colors))
            print('min_colors_size:', min_colors_size)
        self.min_colors_size = min_colors_size

    def solve_forward_checking_rec(self, node, min_colors_size, nodes_colors):
        ...
        for c in range(min_colors_size):
            ...
            if self.check_conflicts(node, nodes_colors):
                if self.solve_forward_checking_rec(node + 1, min_colors_size, nodes_colors):
                    return True
